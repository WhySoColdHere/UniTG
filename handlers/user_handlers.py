...
async def role_schedule_st(message: types.Message, state: FSMContext):
    ...
    await state.set_state(ScheduleStates.waiting_for_role.state)


@dp.message_handler(state=ScheduleStates.waiting_for_role)
async def institute_schedule_st(message: types.Message, state: FSMContext):
    ...
    await state.set_state(ScheduleStates.waiting_for_institute.state)


@dp.message_handler(state=ScheduleStates.waiting_for_institute)
async def course_schedule_st(message: types.Message, state: FSMContext):
    ...
    await state.update_data(institute_st=message.text)
    DATA.append(message.text)

    await bot.send_message(message.chat.id, 'Выбери курс',
                           reply_markup=common_kb.group_keyboard_common_reply(
                               common_keyboard_names.courses()))
    await state.set_state(ScheduleStates.waiting_for_course.state)


@dp.message_handler(state=ScheduleStates.waiting_for_course)
async def education_form_schedule_st(message: types.Message, state: FSMContext):
    await kinda_validator_st(message, common_keyboard_names.courses(), "Выберите существующий курс!")
    ...
    await state.set_state(ScheduleStates.waiting_for_educational_form.state)


@dp.message_handler(state=ScheduleStates.waiting_for_educational_form)
async def group_schedule_st(message: types.Message, state: FSMContext):
    await kinda_validator_st(message, common_keyboard_names.education_forms(),
                             "Выберите существующую форму обучения!")
    ...
    await state.set_state(ScheduleStates.waiting_for_group.state)


@dp.message_handler(state=ScheduleStates.waiting_for_group)
